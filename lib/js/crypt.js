const crypt = {
  ...
  aes(msg, key, mode) {
    ...
    else throw new Error("Invalid key length. Use 16, 24, or 32 bytes.");

    // Minimal AES implementation using standard S-box and key schedule
    ...
    // Rotate, SubWord, and Rcon functions
    function rotWord(w) { return w.slice(1).concat(w[0]); }
    function subWord(w) { return w.map(b => sBox[b]); }
    const Rcon = [0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36];

    // Key expansion
    const w = new Uint8Array(4 * 4 * (Nr + 1));
    key.forEach((b, i) => w[i] = b);
    const temp = new Array(4);
    let i = Nk;
    while (i < 4 * (Nr + 1)) {
      for (let t = 0; t < 4; t++) temp[t] = w[4 * (i - 1) + t];
      if (i % Nk === 0) {
        const rw = rotWord(temp);
        const sw = subWord(rw);
        for (let t = 0; t < 4; t++) temp[t] = sw[t];
        temp[0] ^= Rcon[(i / Nk) - 1];
      } else if (Nk > 6 && i % Nk === 4) {
        const sw = subWord(temp);
        for (let t = 0; t < 4; t++) temp[t] = sw[t];
      }
      for (let t = 0; t < 4; t++) w[4 * i + t] = w[4 * (i - Nk) + t] ^ temp[t];
      i++;
    }

    // Encrypt single 16-byte block (ECB)
    function cipher(input) {
      const state = input.slice();
      const roundKey = (r) => w.slice(16 * r, 16 * (r + 1));

      const addRoundKey = (r) => {
        for (let i = 0; i < 16; i++) state[i] ^= roundKey(r)[i];
      };
      const subBytes = () => {
      ...
      const mixColumns = () => {
        for (let c = 0; c < 4; c++) {
          ...
          for (let i = 0; i < 4; i++) {
            a[i] = state[i + 4 * c];
            b[i] = a[i] & 0x80 ? (a[i] << 1) ^ 0x11b : a[i] << 1;
            b[i] &= 0xFF;
          }
    ...
    if (mode === 0) return cipher(msg);
    else throw new Error("Decrypt not implemented in this minimal example.");
  },

  compare(algo, target, hash) {
