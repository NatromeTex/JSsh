function jsfetch() {
  ...
  function getSysInfo() {
    ...
    return { username, cwd, tty, cpu, ram, gpu, pkg, ver };
  }

  const info = getSysInfo();
  const gpuLabel = info.gpu.vendor !== "unknown"
    ? `${info.gpu.vendor} ${info.gpu.model}`.trim()
    : info.gpu.model;
  const infoLines = [
    ...
    `${CLR_LABEL}GPU:${CLR_RESET}\t${CLR_VALUE}${gpuLabel}${CLR_RESET}`,
